...
void CutestProblem::build_terms() {
  ...
  eliminate_singleton_inequality_rows_.reserve(inequality_terms_.size());
  for (const Term &term : inequality_terms_) {
    eliminate_singleton_inequality_rows_.push_back(
...
const std::uint8_t *CutestProblem::eliminate_singleton_inequality_rows() const {
  return eliminate_singleton_inequality_rows_.data();
}
