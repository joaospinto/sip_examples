...
auto run(const char *runtime_path, const char *problem_library_path,
  ...
  if (use_qp_settings) {
    ...
    settings.termination.max_duality_gap = 1e-6;
  ...
  problem.evaluate_derivatives(problem.initial_x().data(), original_y.data(),
                               original_z.data());
  std::vector<double> qp_linear(model_output.gradient_f,
                                model_output.gradient_f + x_dim);
  std::vector<double> qp_equality_rhs(y_dim);
  std::vector<double> qp_inequality_rhs(s_dim);
  if (use_qp_settings) {
    const auto &initial_x = problem.initial_x();
    ...
    for (int col = 0; col < hessian.cols; ++col) {
      for (int index = hessian.indptr[col]; index < hessian.indptr[col + 1];
        ...
        qp_linear[row] -= value * initial_x[col];
        if (row != col) {
          qp_linear[col] -= value * initial_x[row];
        }
      }
    }
    for (int i = 0; i < y_dim; ++i) {
      qp_equality_rhs[i] = -model_output.c[i];
    }
    for (int i = 0; i < s_dim; ++i) {
      qp_inequality_rhs[i] = -model_output.g[i];
    }
    const auto add_jacobian_product = [&](const sip_qdldl::SparseMatrix &matrix,
                                          std::vector<double> &result) {
      for (int col = 0; col < matrix.cols; ++col) {
        for (int index = matrix.indptr[col]; index < matrix.indptr[col + 1];
             ++index) {
          result[col] += matrix.data[index] * initial_x[matrix.ind[index]];
        }
      }
    };
    add_jacobian_product(model_output.jacobian_c, qp_equality_rhs);
    add_jacobian_product(model_output.jacobian_g, qp_inequality_rhs);
  }
  if (use_qp_settings) {
  ...
  double qpbenchmark_duality_gap = std::numeric_limits<double>::infinity();
  if (scaling_enabled) {
    ...
    problem.evaluate_derivatives(original_x.data(), original_y.data(),
                                 original_z.data());
    double original_complementarity = 0.0;
    int worst_complementarity_index = -1;
    for (int i = 0; i < s_dim; ++i) {
      const double complementarity =
          std::abs(model_output.g[i] * original_z[i]);
      if (complementarity > original_complementarity) {
        original_complementarity = complementarity;
        worst_complementarity_index = i;
      }
    }
    if (std::getenv("SIP_CUTEST_DIAGNOSTICS") != nullptr) {
      double maximum_auxiliary_complementarity = 0.0;
      double mean_auxiliary_complementarity = 0.0;
      double maximum_slack_residual = 0.0;
      int worst_slack_residual_index = -1;
      for (int i = 0; i < s_dim; ++i) {
        const double original_s = workspace.vars.s[i] / scaling.z[i];
        maximum_auxiliary_complementarity =
            std::max(maximum_auxiliary_complementarity,
                     std::abs(original_s * original_z[i]));
        mean_auxiliary_complementarity += original_s * original_z[i];
        const double slack_residual = std::abs(model_output.g[i] + original_s);
        if (slack_residual > maximum_slack_residual) {
          maximum_slack_residual = slack_residual;
          worst_slack_residual_index = i;
        }
      }
      if (s_dim > 0) {
        mean_auxiliary_complementarity /= s_dim;
      }
      std::cerr << "original_complementarity=" << original_complementarity
                << " auxiliary_complementarity="
                << maximum_auxiliary_complementarity
                << " mean_auxiliary_complementarity="
                << mean_auxiliary_complementarity
                << " maximum_slack_residual=" << maximum_slack_residual;
      if (worst_complementarity_index >= 0) {
        const int i = worst_complementarity_index;
        const double original_s = workspace.vars.s[i] / scaling.z[i];
        std::cerr << " worst_index=" << i << " g=" << model_output.g[i]
                  << " s=" << original_s << " z=" << original_z[i]
                  << " g_plus_s=" << model_output.g[i] + original_s
                  << " s_times_z=" << original_s * original_z[i];
      }
      std::cerr << '\n';
      if (worst_slack_residual_index >= 0) {
        const int i = worst_slack_residual_index;
        const int jacobian_nnz = model_output.jacobian_g.indptr[i + 1] -
                                 model_output.jacobian_g.indptr[i];
        const double original_s = workspace.vars.s[i] / scaling.z[i];
        std::cerr << "worst_slack_index=" << i
                  << " jacobian_nnz=" << jacobian_nnz
                  << " g=" << model_output.g[i] << " s=" << original_s
                  << " z=" << original_z[i] << '\n';
      }
    }
    ...
    if (std::getenv("SIP_CUTEST_DIAGNOSTICS") != nullptr) {
      double original_duality_gap = 0.0;
      for (int i = 0; i < x_dim; ++i) {
        original_duality_gap += original_x[i] * dual[i];
      }
      for (int i = 0; i < y_dim; ++i) {
        original_duality_gap -= model_output.c[i] * original_y[i];
      }
      for (int i = 0; i < s_dim; ++i) {
        original_duality_gap -= model_output.g[i] * original_z[i];
      }
      std::cerr << "original_duality_gap=" << std::abs(original_duality_gap)
                << '\n';
    }
    if (use_qp_settings) {
      std::vector<double> hessian_x(x_dim, 0.0);
      const auto &hessian = model_output.upper_hessian_lagrangian;
      for (int col = 0; col < hessian.cols; ++col) {
        for (int index = hessian.indptr[col]; index < hessian.indptr[col + 1];
             ++index) {
          const int row = hessian.ind[index];
          const double value = hessian.data[index];
          hessian_x[row] += value * original_x[col];
          if (row != col) {
            hessian_x[col] += value * original_x[row];
          }
        }
      }
      double x_hessian_x = 0.0;
      double linear_x = 0.0;
      for (int i = 0; i < x_dim; ++i) {
        x_hessian_x += original_x[i] * hessian_x[i];
        linear_x += qp_linear[i] * original_x[i];
      }
      double equality_dual = 0.0;
      for (int i = 0; i < y_dim; ++i) {
        equality_dual += qp_equality_rhs[i] * original_y[i];
      }
      double inequality_dual = 0.0;
      for (int i = 0; i < s_dim; ++i) {
        inequality_dual += qp_inequality_rhs[i] * original_z[i];
      }
      qpbenchmark_duality_gap =
          std::abs(x_hessian_x + linear_x + equality_dual + inequality_dual);
      if (std::getenv("SIP_CUTEST_DIAGNOSTICS") != nullptr) {
        std::cerr << "qpbenchmark_duality_gap=" << qpbenchmark_duality_gap
                  << '\n';
      }
      if (std::getenv("SIP_CUTEST_REQUIRE_QPBENCHMARK_GAP") != nullptr &&
          qpbenchmark_duality_gap >= 1e-6) {
