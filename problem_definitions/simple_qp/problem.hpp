...
constexpr int kQdldlKktNnz = 14;
constexpr int kQdldlKktLNnz = 15;
constexpr int kSlacgLNnz = 7;
constexpr std::array<int, kKktDim> kKktPinv = {6, 5, 4, 3, 2, 1, 0};

inline sip::Settings settings() {
  return sip::Settings{
      .termination =
          {
...
inline void initialize(sip::Workspace &workspace) {
...
inline void configure_qdldl_sparsity(sip_qdldl::ModelCallbackOutput &mco) {
...
inline void evaluate(const sip::ModelCallbackInput &mci, double *f,
                     double *gradient_f, double *c, double *g,
                     double *upper_hessian_lagrangian, double *jacobian_c,
                     double *jacobian_g) {
  *f = 0.5 * (4.0 * mci.x[0] * mci.x[0] +
              2.0 * mci.x[0] * mci.x[1] +
              2.0 * mci.x[1] * mci.x[1]) +
       mci.x[0] + mci.x[1];
